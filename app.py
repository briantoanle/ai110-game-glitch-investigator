...
if "attempts" not in st.session_state:
    #FIX: Attempts must start at 0 so attempts-left math is accurate from the first render.
    st.session_state.attempts = 0
...
if new_game:
    st.session_state.attempts = 0
    #FIX: New game secret must respect selected difficulty range, not a hardcoded global range.
    st.session_state.secret = random.randint(low, high)
    #FIX: Reset all gameplay state on new game (history, score, status, hint) to avoid stale carry-over.
    st.session_state.history = []
    st.session_state.score = 0
    st.session_state.status = "playing"
    st.session_state.hint_message = None
...
if show_hint and st.session_state.hint_message:
    #FIX: Render hint outside submit branch so it remains visible after automatic rerun.
